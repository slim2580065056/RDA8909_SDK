...
SELECTED_APNCFG := $(filter $(VALID_YESNO_LIST), $(CT_APNCFG))
ifeq "$(SELECTED_APNCFG)" ""
    ifneq "${CT_APNCFG}" ""
        $(error "!!!! CT_APNCFG=${CT_APNCFG} - Not a valid resource generation option !!!!")
    endif
endif
...
SELECTED_FSGEN := $(filter $(VALID_YESNO_LIST), $(CT_FSGEN))
ifeq "$(SELECTED_FSGEN)" ""
    ifneq "${CT_FSGEN}" ""
        $(error "!!!! CT_FSGEN=${CT_FSGEN} - Not a valid user sector generation option !!!!")
    endif
endif
...
export CAT := cat
export LZMA := lzma
export DUAL_BOOT_MERGE := python ${SOFT_WORKDIR}/env/utils/dual_boot_merge.py
export LOD_MERGE := python ${SOFT_WORKDIR}/env/utils/lod_merge.py
export SDKGEN := sdkgen
export FSGEN_TOOL := flashfstools
...
export LIBGCC_PATH := $(shell sh -c "dirname $(LIBGCC_FILE_NAME)")
else
...
# ----------------------------------------------------------------------
# The convention for LOCAL_LIBS is to declare them starting from $SOFT_WORKDIR
...
RAM_EXTAPP_SIZE ?= 0x10000
RAM_BTFW_SIZE ?= 0

ifeq "${FOTA_SUPPORT}" "y"
CODE_FLASH_PHY_END ?= ${FLASH_BASE}+${FOTA_BACKUP_AREA_START}-${FLASH_EXTAPP_SIZE}
else
CODE_FLASH_PHY_END ?= ${FLASH_BASE}+${SPIFFS_FLASH_START}-${FLASH_EXTAPP_SIZE}
endif
...
LDPPFLAGS += \
    ...
    -DFLASH_EXTAPP_SIZE=${FLASH_EXTAPP_SIZE} \
    -DSPIFFS_FLASH_BASE=${FLASH_BASE}+${SPIFFS_FLASH_START} \
    -DSPIFFS_FLASH_SIZE=${SPIFFS_FLASH_SIZE} \
    -DUSER_DATA_PHY_BASE=${FLASH_BASE}+${USER_DATA_BASE} \
    -DUSER_DATA_SIZE=${USER_DATA_SIZE} \
    -DFLASH_SIZE=${FLASH_SIZE} \
    -DRAM_SIZE=${RAM_SIZE} \
    -DRAM_EXTAPP_SIZE=${RAM_EXTAPP_SIZE} \
    -DRAM_BTFW_SIZE=${RAM_BTFW_SIZE} \
...
ifeq "${CT_CUSTSECTOR}" "y"
LDPPFLAGS += \
    -DUSER_RES_DATA_BASE=${FLASH_BASE}+${USER_RES_DATA_BASE} \
    -DUSER_RES_DATA_SIZE=${USER_RES_DATA_SIZE} \
    -DCT_CUSTSECTOR
endif
else # CHIP_HAS_AP
LDPPFLAGS += \
...
ifeq "${LTE_NBIOT_SUPPORT}" "y"
LDPPFLAGS += \
    -DCALIB_RFNB_BASE=${FLASH_BASE}+${CALIB_RFNB_BASE} \
    -DLTE_NBIOT_SUPPORT \
    -DNB_CODE_IN_PSRAM=1
endif
...
LDPPFLAGS += \
	...
	-DNB_SRAM_STACK_SIZE=${NB_SRAM_STACK_SIZE}
endif


ifeq "${BUILD_BCPU_PSRAM}" "y"
...
ifeq "${CHIP_HAS_MMU}"  "1"
LDPPFLAGS += \
    -DCHIP_HAS_MMU=${CHIP_HAS_MMU} \
    -DEXT_UTLB_BASE=${EXT_UTLB_BASE} \
    -DEXT_UTLB_SIZE=${EXT_UTLB_SIZE}
endif
#--warn-section-align
...
BCPU_BIN_FINAL := ${BCPU_BAS_FINAL}.bcpu.elf
BCPU_MAP_FINAL := ${BCPU_BAS_FINAL}.bcpu.map
endif
...
USR_DATA_OBJECTS += ${BIN_PATH}/user.bin.o  ${BIN_PATH}/spiffs_user.bin.o
endif
endif
...
${BIN_PATH}/%.bin.lzma.o ${BIN_PATH}/%.sum.o: ${BIN_STAGE1} | ${BIN_PATH}
	...
	@${ECHO}
	@${ECHO} "cksum             $*"
	${CKSUM}  ${BIN_PATH}/$*.bin.org |cut -d " " -f 1  | perl -e 'binmode(STDOUT);print pack "J*", <>' > ${BIN_PATH}/$*.sum
...
${BIN_PATH}/spiffs_user.bin.o: ${BIN_PATH}/usrgenerator/spiffs_user.bin
	${OBJCOPY} -I binary -O elf32-littlemips -B mips --rename-section .data=.spiffs_user_data,alloc,load,readonly,data,contents $< $@
endif
...
LOCAL_LIB_INCR_LINK_EXPORT ?= no

ifeq "${LOCAL_LIB_INCR_LINK_EXPORT}" "yes"
$(LOCAL_SRCLIBRARY): $(LOCAL_LIB_INCR_LINK_EXPORT_SYMBOLS)
endif

LOCAL_SRCLIBRARY_INCR_OBJECT := $(OBJ_REL_PATH)/$(MODULE_NAME)lib.o
...
ifneq "$(FULL_SRC_OBJECTS)" ""
...
else ifeq "${LOCAL_LIB_INCR_LINK_EXPORT}" "yes"
	...
	then \
		$(LD) -EL -r @${LOCAL_SRCLIBRARY}.l -o $(LOCAL_SRCLIBRARY_INCR_OBJECT); \
		$(OBJCOPY) --keep-global-symbols=${LOCAL_LIB_INCR_LINK_EXPORT_SYMBOLS} $(LOCAL_SRCLIBRARY_INCR_OBJECT); \
		$(AR) cru ${LOCAL_SRCLIBRARY} $(LOCAL_SRCLIBRARY_INCR_OBJECT); \
...
ifeq "${LOCAL_LIB_INCR_LINK}" "yes"
	$(LD) $(OFORMAT) -r -o $(LOCAL_SRCLIBRARY_INCR_OBJECT) $^
	$(STRIP) --strip-unneeded -x $(LOCAL_SRCLIBRARY_INCR_OBJECT)
	$(AR) cr $@ $(LOCAL_SRCLIBRARY_INCR_OBJECT)
else ifeq "${LOCAL_LIB_INCR_LINK_EXPORT}" "yes"
	$(LD) $(OFORMAT) -r ${FULL_SRC_OBJECTS} -o $(LOCAL_SRCLIBRARY_INCR_OBJECT)
	$(OBJCOPY) --keep-global-symbols=${LOCAL_LIB_INCR_LINK_EXPORT_SYMBOLS} $(LOCAL_SRCLIBRARY_INCR_OBJECT)
	$(AR) cru ${LOCAL_SRCLIBRARY} $(LOCAL_SRCLIBRARY_INCR_OBJECT)
else
...
APN_DATA := ${SOFT_WORKDIR}/application/
...
apncfg : 
	mkdir -p ${RESGEN_DIR}
	chmod 777 ${RESGEN_DIR}
	chmod 777 ${SOFT_WORKDIR}/at/ats/apn/resources/resgen.sh

# Caution: no make's built-in implicit rules for the resource generator either
	+${SOFT_WORKDIR}/at/ats/apn/resources/resgen.sh ${SOFT_WORKDIR}/at/ats/apn ${CT_ERES}
...
FS_ROOT_LOD := $(BINARY_PATH)/fsroot_$(LODBASE_NO_PATH)_flash.lod
FS_ROOT_BIN := $(BINARY_PATH)/fsroot_$(LODBASE_NO_PATH)_flash.bin

.PHONY: fsgen
ifeq ("$(USE_LEGACY_SPIFFS)", "y")
FSGEN_TOOL_DIR := ${SOFT_WORKDIR}/toolpool/spiffs_tool
fsgen: | ${BINARY_PATH}
	@${MAKE} -C $(FSGEN_TOOL_DIR) clean
	@${MAKE} -C $(FSGEN_TOOL_DIR) mkspiffs
	@$(FSGEN_TOOL_DIR)/mkspiffs \
		--flash_size=$(FLASH_SIZE) \
		--vfs_area_start=$(SPIFFS_FLASH_START) \
		--vfs_area_size=$(SPIFFS_FLASH_SIZE) \
		--output="$(FS_ROOT_LOD)" \
		--pack_path="$(CT_FSGEN_ROOT)" \
		--pack_spath="$(CT_FSGEN_SROOT)"
else
fsgen: | ${BINARY_PATH}
	@$(FSGEN_TOOL) sffsgen \
		--flash $(SPIFFS_FLASH_START),$(SPIFFS_FLASH_SIZE),$(SFFS_FLASH_EB_SIZE),$(SFFS_FLASH_PB_SIZE) \
		--partition "0,,$(CT_FSGEN_ROOT)" \
		--bin "$(FS_ROOT_BIN)" \
		--lod "$(FS_ROOT_LOD)" \
		--flash-phys-address $(FLASH_PHYS_BASE)
endif
...
targetgen: force
	...
	@${ECHO} "RAM_EXTAPP_SIZE:=      $(RAM_EXTAPP_SIZE)"        >> $(TARGET_FILE)
	@${ECHO} "FLASH_EXTAPP_SIZE:=    $(FLASH_EXTAPP_SIZE)"      >> $(TARGET_FILE)
...
GEN_COMPRESS_ELF_SECTION_ADDR_FILE_FUNC = ${OBJDUMP} -h ${1} | perl -e ' \
    while(<>) { \
        if (/^ *[0-9]+ +([^ ]+) +([0-9a-zA-Z]+) +([0-9a-zA-Z]+) +([0-9a-zA-Z]+) /) { \
            ...
            } elsif (($$secstr ne ".user_data") && ($$secstr ne ".spiffs_user_data")) { \
...
ifeq "${LINK_SDK_API_STUB}" "y"
LD_OPTIONAL_OBJECTS += ${SDK_API_STUB_OBJECT}
...
$(error Cannot build BCPU image. Please add "export NO_BCPU_IMAGE := y" in your Makefile)
endif
endif

NATIVE_MAP_FILE := $(call NOCYGPATH,${MAP_FILE})
NATIVE_HEX := $(call NOCYGPATH,${HEX})
NATIVE_LODBASE := $(call NOCYGPATH,${LODBASE})
NATIVE_BAS := $(call NOCYGPATH,${BAS})
NATIVE_TARGET_FILE := $(call NOCYGPATH,${TARGET_FILE})
...
#	#
			...
			srecmap -c ${NATIVE_MAP_FILE} -m rom ${NATIVE_HEX} ${NATIVE_LODBASE} ${STDOUT_NULL}
			@${ECHO}
			@${ECHO} "---------------------------------------------------"
...
ifeq "${CT_ASICSIMU}" "1"
		...
		srecmap -c ${NATIVE_MAP_FILE} -m simu ${NATIVE_HEX} ${NATIVE_LODBASE} ${STDOUT_NULL}
#	#
else # !CT_ASICSIMU}
#	#
ifeq "${CT_RAMRUN}" "1"
...
ifeq "${BUILD_BOOT_IMAGE}" "1"
				srecmap -c ${NATIVE_MAP_FILE} -m ramrunImage ${NATIVE_HEX} ${NATIVE_BAS}_ ${STDOUT_NULL}
				rm -f ${BINARY_PATH}/*.img
				${MAKE} ramrun_bootimage GEN_RAMRUN_IMAGE=1
else
				srecmap -c ${NATIVE_MAP_FILE} -m preload ${NATIVE_HEX} ${NATIVE_LODBASE} ${STDOUT_NULL}
...
#	#
				...
				srecmap -c ${NATIVE_MAP_FILE} -m ${FLSH_MODEL} ${NATIVE_HEX} ${NATIVE_BAS}_ ${STDOUT_NULL}
				@${ECHO}
				@${ECHO} "---------------------------------------------------"
...
ifeq "${EXPORT_BOARD_INFO}" "y"
				${MAKE} targetgen
ifeq "${CT_BIN}" "1"
				srecmapbin -c ${NATIVE_MAP_FILE} -m ${FLSH_MODEL} -b ${NATIVE_TARGET_FILE} ${NATIVE_HEX} ${NATIVE_LODBASE} ${STDOUT_NULL}
else
				srecmap -c ${NATIVE_MAP_FILE} -m ${FLSH_MODEL} -b ${NATIVE_TARGET_FILE} ${NATIVE_HEX} ${NATIVE_LODBASE} ${STDOUT_NULL}
endif
else
ifeq "${CT_BIN}" "1"
				srecmapbin -c ${NATIVE_MAP_FILE}) -m ${FLSH_MODEL} ${NATIVE_HEX} ${NATIVE_LODBASE} ${STDOUT_NULL}
else
				srecmap -c ${NATIVE_MAP_FILE} -m ${FLSH_MODEL} -b ${NATIVE_TARGET_FILE} ${NATIVE_HEX} ${NATIVE_LODBASE} ${STDOUT_NULL}
endif
endif
...
dbmerge: | lod
	@${ECHO}
	@${ECHO} "MERGE             merge_${LODBASE_NO_PATH}_flash.lod"
	@${ECHO}
	${DUAL_BOOT_MERGE} --bl="${BINARY_PATH}/blfota_${LODBASE_NO_PATH}_flash.lod" \
		--lod="${BINARY_PATH}/${LODBASE_NO_PATH}_flash.lod" \
		--output="${BINARY_PATH}/merge_${LODBASE_NO_PATH}_flash.lod"
	@${ECHO} "---------------------------------------------------"
	@${ECHO} "merge_${LODBASE_NO_PATH}_flash.lod Generated"
	@${ECHO} "---------------------------------------------------"


##################################################################

FULL_LOD := ${BINARY_PATH}/full_${LODBASE_NO_PATH}_flash.lod
LOD_MERGE_INPUT := --input="${FULL_LOD}"

# if no bootloader, change base
ifeq (${FLASH_BOOT_OFFSET}, 0)
	LOD_MERGE_INPUT := --input="${BINARY_PATH}/${LODBASE_NO_PATH}_flash.lod"
endif

NEED_MERGE ?=
ifeq ("${FSGEN}", "yes")
	NEED_MERGE += ${FS_ROOT_LOD}
endif
LOD_MERGE_INPUT += $(foreach n, ${NEED_MERGE}, --input="${n}")

.PHONY: fullmerge
fullmerge: | lod
# merge bootloader
ifneq (${FLASH_BOOT_OFFSET}, 0)
	@${ECHO} "- merge blfota_${LODBASE_NO_PATH}_flash.lod"
	${DUAL_BOOT_MERGE} --bl="${BINARY_PATH}/blfota_${LODBASE_NO_PATH}_flash.lod" \
		--lod="${BINARY_PATH}/${LODBASE_NO_PATH}_flash.lod" \
		--output="${FULL_LOD}"
endif
# merge others
ifneq ("${NEED_MERGE}", "")
	@${ECHO} "- merge $(foreach n,${NEED_MERGE},$(notdir ${n}))"
	@${LOD_MERGE} ${LOD_MERGE_INPUT} --output="${FULL_LOD}"
endif
	@${ECHO} "---------------------------------------------------"
	@${ECHO} "$(notdir ${FULL_LOD}) Generated"
	@${ECHO} "---------------------------------------------------"
