...
extern u32 osi_is_wakeup_from_deep_sleep();
#define  CFW_PM3_FLAG    osi_is_wakeup_from_deep_sleep()
extern BOOL gUsimReInsertFlag;

extern bool osi_is_wakeup_from_pm2();
#define CFW_PM2_FLAG osi_is_wakeup_from_pm2()
...
BOOL CFW_GetSimAID(UINT8 *aid, UINT8 *length, CFW_SIM_ID nSimID);
BOOL CFW_SimAIDPresent(CFW_SIM_ID nSimID);
...
BOOL SRVAPI CFW_CfgCurrentOperatorAvail(
    UINT8  pOperatorId[6],
    UINT8 format,
    UINT8 *pName);

VOID SRVAPI CFW_CfgNwSetOperatorStatus(
    UINT8 reset,
    UINT8 pOperatorId[6]);
...
typedef struct _CFW_SAT_OPENCHANNEL_RSP //for proactive command 0x15
{
    ...
    UINT8 Duration1Unit;
    UINT8 Duration1Interval;

    UINT8 Duration2Unit;
    UINT8 Duration2Interval;

    UINT8 BearerType;
    UINT8 BearerLen;
    UINT8 *BearerDescription;

    UINT16 BufferSize;

    UINT8 OtherAddressLen;
    UINT8 OtherAddressType;
    UINT8 *OtherAddress;
    ...
    UINT8 UserPasswordLen;
    UINT8 UserPasswordDcs;
    UINT8 *UserPassword;

    UINT8 TransportLevelType;
    UINT16 TransportLevelPort;

    UINT8 DestAddressLen;
    UINT8 DestAddressType;
    UINT8 *DestAddress;

    UINT8 NetworkAccessNameLen;
...
UINT32 SRVAPI CFW_GprsGetPdpAddr(
    ...
    UINT8 *pPdpAdd, CFW_SIM_ID nSimID);
...
UINT32 CFW_GprsAtt(
    ...
    UINT16 nUTI,
    CFW_SIM_ID nSimID);
UINT32 CFW_GprsAttEX(UINT8 nState, UINT16 nUTI, CFW_SIM_ID nSimID, COS_CALLBACK_FUNC_T func);
...
UINT32 CFW_EmodReadBindSIM(CFW_EMOD_READ_BIND_SIM *pReadBindSim);
...
typedef struct _CFW_ALL_CELL_INFO
{
    UINT8                       nAllCellNUM;    //all cell number.
    CFW_TSM_NEIGHBOR_CELL_INFO *pCellInfos;    //use to get all cell info,AT or MMI need free this handle.
}CFW_ALL_CELL_INFO;
/***************************************API FUNCTION OF TSM***********************************************/
...
#ifdef LTE_NBIOT_SUPPORT
typedef struct CFW_CMCCTEST_PSMDATA_Tag
{
    uint8_t  host[50];
    uint8_t     type;
    uint16_t    port;
...
UINT32 CFW_GprsCtxEpsModify(UINT16 nUTI, UINT8 nCid, UINT8 ueTriggerLocalCause, CFW_SIM_ID nSimID);
#endif //LTE_NBIOT_SUPPORT
