...
#define HAL_MAX_PM1_TIME 0xffffffff

#define HAL_PMU_CFG_MARK_POS 1
#define HAL_PMU_CFG_MARK_MSK (1 << HAL_PMU_CFG_MARK_POS)
#define HAL_PMU_CFG_MARK (1 << HAL_PMU_CFG_MARK_POS)

#define HAL_PMU_CFG_STORAGE_POS 2
#define HAL_PMU_CFG_STORAGE_MSK ((u16)0x3 << HAL_PMU_CFG_STORAGE_POS)
#define HAL_PMU_CFG_FLASH (1 << HAL_PMU_CFG_STORAGE_POS)
#define HAL_PMU_CFG_AON   (2 << HAL_PMU_CFG_STORAGE_POS)

#define HAL_LSP_TRACE(level, tstmap, format, ...) \
    SXS_TRACE(_HAL | TDB | tstmap, format, ##__VA_ARGS__)
...
typedef enum {
    ...
    NUM_OF_LPS_SLEEP_STATUS,
} HAL_LPS_SLEEP_STATUS;

typedef enum {
    LPS_GET_NB_TIMER = 0,

    NUM_OF_LPS_CB_WITH_RET = LPS_GET_NB_TIMER + 1,
} HAL_LPS_WITH_RET_CB;
...
typedef u32 HAL_LPS_CB_WITHRET_T(VOID);

typedef VOID HAL_LPS_CALLBACK_FUNC(VOID *user_data, HAL_LPS_SLEEP_STATUS status);
...
#else
PUBLIC VOID hal_LpsOpen(CONST UINT32 wakestatus);
#endif
// =============================================================================
// hal_LpsClose
// -----------------------------------------------------------------------------
...
PUBLIC VOID hal_LpsRegisterCallBackFunc(UINT8 *module_name, HAL_LPS_CALLBACK_FUNC *func, VOID *user_data);
PUBLIC VOID hal_LpsUnRegisterCallBackFunc(HAL_LPS_CALLBACK_FUNC *func, VOID *user_data);

#ifdef CHIP_DIE_8909
PUBLIC VOID hal_LpsSetSleepCallback(HAL_LPS_SLEEP_STATUS status, HAL_LPS_CALLBACK_T *cbFunc);
PUBLIC VOID hal_LpsSetWithRetCallback(HAL_LPS_WITH_RET_CB status, HAL_LPS_CB_WITHRET_T *cbFunc);
...
UINT8 hal_LpsIsReadyForPM3();
VOID *hal_LpsGetCtx(u32 *length);
PUBLIC VOID hal_AON_PowerOn();
PUBLIC VOID hal_AON_PowerOff();
PUBLIC UINT32 hal_PM3_Get_Storage();
PUBLIC VOID hal_PM3_Set_Storage(UINT32 sto);
PUBLIC UINT32 hal_PM3_Get_Mark();
PUBLIC VOID hal_PM3_Mark_Clr();
PUBLIC VOID hal_PM3_Mark();
PUBLIC VOID hal_PM3_SetHwtime(UINT32 hw_time);
PUBLIC UINT32 hal_PM3_GetHwtime();
PUBLIC UINT32 hal_is_PM3_Reset();
PUBLIC BOOL hal_is_PM2_Reset();
PUBLIC BOOL hal_is_PM2_Restart();

PUBLIC VOID hal_LpsEnableRC32k();

struct LPS_CONFIG_DATA {
    UINT16 pm2_idle;
};
void hal_LpsSetPM2IdleTime(UINT16 idletime);
struct LPS_CONFIG_DATA* hal_LpsGetLpsConfigData(void);
#endif
